...
- **Entities (Phase 2):** `Get/New/Update/Remove-GuacConnection` (`-ResolveParentGroupName` on the Get, adds `ParentGroupName`; `-Name` and `-Protocol` filters on the Get), `-GuacConnectionGroup` (`-ResolveParentGroupName` on the Get, adds `ParentGroupName`; `-Name` filter on the Get; tree retrieval moved to `Get-GuacConnectionGroupTree`), `Get-GuacConnectionGroupTree` (hierarchical tree via `connectionGroups/{id}/tree`, `-Permission` filters connections within the tree), `-GuacUser` (`-Permissions`/`-EffectivePermissions` on the Get; `-Name` filter on the Get, matches against username), `-GuacUserGroup` (`-Name` filter on the Get, matches against identifier), `-GuacSharingProfile` (`-ResolveConnectionName` on the Get, adds `PrimaryConnectionName`; `-Name` filter on the Get)
